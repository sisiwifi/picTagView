...
当前版本已经演进为一套完整的本地图库工作流：导入、去重、日期浏览、相册浏览、搜索、收藏占位、选择模式批处理、标签元数据编辑、主分类管理、回收站复用 BrowsePage 的 trash contract，以及多级缩略图缓存与刷新修复。BrowsePage 通过页面契约切换日历浏览与回收站浏览，并按页面横竖屏方向切换瀑布流布局：横屏保持等高 justified flow，竖屏切换为等宽 masonry；滚动浏览与分页浏览都由同一套页面配置驱动，滚动模式还支持固定的窗口范围配置 `40-200`，默认 `100`。
...
| 模块 | 当前能力 |
| --- | --- |
| 导入与去重 | 支持文件夹导入，按日期写入 `media/YYYY-MM/`；维护 SHA-256 与 quick hash；刷新和恢复链路复用相同去重规则；切换页面不会打断 Gallery 导入，可手动停止当前导入 |
| 日期与相册浏览 | 支持 CalendarOverview、按月份浏览、按相册路径浏览；相册显示状态、计数与封面由可见图片派生 |
| 页面配置与分页浏览 | 设置页可切换“滚动浏览 / 分页浏览”，并在滚动模式下设置固定窗口范围 `40-200`（默认 `100`）；BrowsePage 通过页面契约承载日历浏览与回收站浏览，会在滚动/分页两种模式下按页面方向自动切换瀑布流布局，横屏等高、竖屏等宽，并在缩放后尽量保持当前首张可见图附近 |
| 选择模式 | 支持瀑布流 / 网格 / 列表切换、窗口化渲染、Ctrl/Shift 多选、拖选、详情浮层与批量操作；详情浮层可直接编辑图片文件名、主分类和创建时间，其中主分类/创建时间支持多选批量应用，文件名/主分类修改会直接回写当前列表而不整页重载，跨月份改时间时会同步物理移动文件并仅对当前列表做局部移除与重排；右下角操作岛在窄屏下保持单行紧凑布局，并可向页面边缘收拢 |
...
| 搜索与收藏 | 支持单输入文件名 / Tag / 路径 quick hash 搜索；收藏一级页已预留占位 |
...
| 回收站 | 支持图片/相册移入回收站、批量还原、彻底删除、清空回收站；回收站页复用选择模式与详情浮层交互，并在首屏展示后静默执行对账与缺失 cache 修复 |
| 缩略图与缓存 | 使用 `backend/temp/` 管理月份封面/临时预览，使用 `backend/data/cache/` 管理浏览缓存缩略图 |
| 刷新与修复 | 支持库刷新、孤立数据清理、缺失预览补齐、未入库媒体收编、哈希索引重建；BrowsePage 的日历与回收站契约在 cache 或 temp 预览 404 时会触发定向 quick refresh 并回写最新元数据 |
...
│   │   │   └── ThumbCard.vue               # 缩略图卡片
│   │   ├── pages/                    # 页面视图
│   │   │   ├── BrowsePage.vue        # 月份 / 相册浏览主页面；支持滚动浏览与分页浏览，并会按横竖屏切换等高 / 等宽瀑布流
│   │   │   ├── CalendarOverview.vue  # 日期总览页
│   │   │   ├── CategorySettingsPage.vue    # 主分类设置页
│   │   │   ├── EmptyPage.vue        # 空状态占位页
│   │   │   ├── GalleryPage.vue      # 导入与图库入口页
│   │   │   ├── HomePage.vue         # 首页概览
│   │   │   ├── SearchPage.vue       # 搜索页（文件名 / Tag / 路径 quick hash）
│   │   │   ├── FavoritesPage.vue    # 收藏页占位
│   │   │   ├── SettingsPage.vue     # 系统设置页
│   │   │   ├── TopLevelPageHeader.vue # 一级页公共页头
│   │   │   └── topLevelPageConvention.js # 一级页导航与搜索约定
│   │   ├── router/                  # 路由配置
│   │   │   └── index.js             # 路由入口
│   │   ├── utils/                   # 前端工具函数
│   │   │   ├── pageConfig.js        # 页面浏览模式设置工具
│   │   │   ├── commonBrowsePage.js   # 统一浏览页契约与数据适配器
...
- `services/app_settings_service.py`
  - 应用设置持久化到 `backend/data/app_settings.json`，包括缓存尺寸、月份封面尺寸、文件名匹配配置、页面浏览模式与滚动窗口范围。
...
- `CalendarOverview.vue`
  - 日期总览页。
- `BrowsePage.vue`
  - 月份/相册浏览主页面，也是选择模式、Tag 菜单和标签编辑的核心页面；同时承载回收站 contract，并支持滚动浏览与分页浏览两种模式，会按横竖屏切换等高 / 等宽瀑布流。
- `CategorySettingsPage.vue`
  - 主分类配置页。
- `SettingsPage.vue`
  - 系统设置、页面配置、标签导入导出、查看器配置等。
- 回收站 contract
  - 已由 `BrowsePage.vue` 的 `trash` contract 接管。
