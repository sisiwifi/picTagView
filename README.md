...
| **选择模式** | 日期详情页/相册页可切换到选择模式，使用固定比例媒体卡片；图片区优先显示 `backend/data/cache` 缩略图，横屏 5 列、竖屏 3 列，支持 Ctrl/Shift、多选拖选与右下角操作岛 |
| **列表选择** | 列表模式支持长按进入选择，沿用与选择界面一致的选择逻辑与圆形选择符号；相册项标识改为缩略图右侧的圆角矩形标签 |
...
```json
{
  "date_group": "2025-03",
  "items": [
    { "type": "image", "name": "photo.jpg", "thumb_url": "/thumbnails/abc.webp", "id": 1, "tags": [23, 45] },
    { "type": "album", "name": "旅行", "thumb_url": "/thumbnails/def.webp", "count": 42, "public_id": "album_1", "tags": [] }
  ]
}
```
...
```json
{
  ...
  "items": [
    { "type": "album", "name": "Day1", "thumb_url": "...", "count": 20, "public_id": "album_2" },
    { "type": "image", "name": "photo.jpg", "thumb_url": "...", "id": 5 }
  ]
}
```
