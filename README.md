...
| 功能 | 说明 |
|------|------|
| **文件夹导入** | 支持多行文件夹导入与主分类选择，按 groupByDate 规则整理到 `media/`；导入过程中切换到其他页面不会中断队列，返回 `/gallery` 可继续查看进度 |
...
| **选择模式** | 日期详情页/相册页可切换到选择模式，使用固定比例媒体卡片；图片区优先显示 `backend/data/cache` 缩略图，横屏 5 列、竖屏 3 列，支持 Ctrl/Shift、多选拖选、卡片详情浮层、右下角操作岛与混合类型按需全选，并对可视区外卡片做窗口化渲染；Tag 菜单改为草稿确认模型，仅在点击“确定”后才批量提交，取消/关闭不会回写；切换浏览态 / 选择态时会按视觉锚点恢复到同一内容附近，避免“迷路” |
...
| **Tag 显色方案（元数据）** | Tag 颜色、边框色、背景色保存在 `Tag.metadata`；由用户或导入数据自行维护，前端详情浮层仅消费元数据显色 |
...
| **回收站** | 设置页右上角提供更醒目的入口进入独立回收站页面；支持把图片或整棵相册目录移入 `trash/`、批量还原、彻底删除、“清空回收站”；进入页面时以后端批量轻量对账补齐预览，删除/还原确认统一改为页面中部弹窗；详情浮层 Tag 区与浏览页保持同款 chip 显色样式，但仅作只读展示 |
...
- TrashPage 现在和 BrowsePage 一样，会在瀑布流与选择模式之间保存当前视觉锚点，并在切换后把同一条目恢复到首屏附近；选择态列表也改为按可视窗口虚拟渲染。
- 回收站不接入 `/api/thumbnails/cache` generation 队列，因为 `/api/trash/items` 已直接返回可复用的 `cache_thumb_url` 与宽高；前端只保留精确宽度的排布缓存，以及元数据缺失时的 `img.onload` 尺寸兜底。
...
- 选择模式详情浮层中的 Tag 行末提供 `+` 按钮，点击后打开 Tag 标签菜单。
- Tag 菜单已拆分为独立组件，支持在其他页面复用。
- 菜单顶部“现有标签”在多选时显示所选图片的公共标签，并支持逐条 `x` 移除。
- 输入框为空时，建议容器固定尺寸显示最近使用的 5 个标签（按 `last_used_at` 倒序）。
- 输入框有内容时，按 `name/display_name` 搜索标签建议；每条建议右侧提供 `+`（写入当前草稿）和笔形按钮（打开标签元数据编辑弹窗）。
- 菜单底部提供“添加新标签”入口，会先向后端预占一个真实 `public_id`，再进入新增弹窗。
- 菜单内提供“自动标签”按钮，调用文件名匹配做预分析并合并到草稿，不会立即回写。
- 菜单右下角提供“取消 / 确定”：取消或关闭会丢弃草稿；点击确定才会批量提交到后端。
- 文件名批量匹配接口仍保留（`/api/images/tags/filename-match`），用于按文件名自动分析并回写。
- 文件名匹配规则按空格分词；下划线 `_` 视为词内字符，不会被拆分。
- Tag 结果排序统一按 `tag.name` 字母顺序。
- Tag 新增/编辑弹窗支持 `public_id` 只读展示、`name` 正则校验、`type` 下拉、Tag 预览、7 个预设样式，以及 HEX8 + HSV + 透明度取色。
- 以下显色字段保存在 `Tag.metadata`，并作为前端唯一显色来源，统一使用 HEX8：
  - `color`
  - `border_color`
  - `background_color`
- 后端不再按 Tag ID 或固定色盘自动分配颜色；新增或已有 Tag 的显色字段需要由用户或导入数据显式维护。
...
说明：`tags` 为图片条目关联的 Tag ID 列表。BrowsePage 普通模式与选择模式都会优先使用 `cache_thumb_url` 或 `thumb_url` 显示缩略图；若缓存尚未生成，前端先显示骨架占位，再按统一的 `/api/thumbnails/cache` generation 队列与 `cursor` 增量轮询链路补齐。前端仅在用户把选择模式信息区切换到 Tag 显示时，才会去重后批量调用 `/api/tags?ids=...` 拉取展示名称，避免普通浏览时产生额外查询。
...
补充：图片条目与相册条目现在都会额外返回 `width` / `height`。BrowsePage 的“大缩略图”照片墙会优先使用这组后端元数据初始化拼图布局，并按“页面标识 + 排序签名 + 精确容器宽度 + 布局指纹”缓存排布结果；前端 `onLoad` 只在宽高缺失时做异常兜底，避免把图片实际加载当作常规排布来源。

补充：BrowsePage 当前只对列表模式与选择模式启用窗口化渲染；“大缩略图”照片墙仍保留全量行布局，但滚动预取逻辑已升级为“视觉锚点 / 缓存锚点”分离：视觉锚点负责模式切换定位恢复，缓存锚点负责触发当前首排与前后 50 张的缓存生成。
...
### 图库管理 `/gallery`
- **选择图片文件夹并导入**：支持多行目录队列与每行主分类选择，使用 `webkitdirectory` 选择目录后按 groupByDate 规则顺序分批上传；根目录直下文件与嵌套子目录统一按 50 个文件切块提交，并显示实时进度。
- **切换页面不中断导入**：`/gallery` 页面在前端路由层保活，导入中切换到首页、日历或设置页不会销毁当前导入队列；返回后会继续显示当前目录与累计进度。
...
- 瀑布流和选择模式切换时会复用和 BrowsePage 相同的视觉锚点恢复策略，避免切换后迷失当前内容
- 瀑布流排布按“排序签名 + 精确容器宽度 + 尺寸指纹”做内存缓存；选择态卡片按可视窗口虚拟化，回收站大列表不会一次性全量挂载
