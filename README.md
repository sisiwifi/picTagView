...
| **Tag 显色方案（元数据）** | Tag 颜色、边框色、背景色统一写入 `Tag.metadata`；按 Tag ID 升序使用 7 色循环分配，前端详情浮层仅消费元数据显色 |
...
- 输入框有内容时，按 `name/display_name` 搜索标签建议；每条建议右侧提供 `+`（写入当前草稿）和笔形按钮（编辑元数据占位）。
- 菜单底部保留“添加新标签（预留）”入口；原“编辑标签”位置已替换为“自动标签”。
...
- 后端会把以下显色字段写入 `Tag.metadata`，并作为前端唯一显色来源：
  - `color`
  - `border_color`
  - `background_color`
- 初版配色按 Tag ID 升序轮回分配（7 色循环）：
  - `#FF0000`
  - `#FF6600`
  - `#C9A600`（对应黄色可读性优化）
  - `#00B84A`（对应绿色可读性优化）
