...
当前版本已经演进为一套完整的本地图库工作流：导入、去重、日期浏览、相册浏览、选择模式批处理、标签元数据编辑、主分类管理、独立回收站，以及多级缩略图缓存与刷新修复。BrowsePage 与 TrashPage 会按页面横竖屏方向切换瀑布流布局：横屏保持等高 justified flow，竖屏切换为等宽 masonry；滚动浏览与分页浏览都由同一套页面配置驱动，滚动模式还支持固定的窗口范围配置 `40-200`，默认 `100`。
...
| 页面配置与分页浏览 | 设置页可切换“滚动浏览 / 分页浏览”，并在滚动模式下设置固定窗口范围 `40-200`（默认 `100`）；BrowsePage 与 TrashPage 会在滚动/分页两种模式下按页面方向自动切换瀑布流布局，横屏等高、竖屏等宽，并在缩放后尽量保持当前首张可见图附近 |
...
| 刷新与修复 | 支持库刷新、孤立数据清理、缺失预览补齐、未入库媒体收编、哈希索引重建；Browse/Trash 页面在 cache 或 temp 预览 404 时会触发定向 quick refresh 并回写最新元数据 |
...
│   │   │   └── TrashPage.vue        # 回收站页；支持滚动浏览与分页浏览，并会按横竖屏切换等高 / 等宽瀑布流
│   │   ├── router/                  # 路由配置
│   │   │   └── index.js             # 路由入口
│   │   ├── utils/                   # 前端工具函数
...
- `CalendarOverview.vue`
  - 日期总览页。
- `BrowsePage.vue`
  - 月份/相册浏览主页面，也是选择模式、Tag 菜单和标签编辑的核心页面；支持滚动浏览与分页浏览两种模式，并会按横竖屏切换等高 / 等宽瀑布流。
- `CategorySettingsPage.vue`
  - 主分类配置页。
- `SettingsPage.vue`
  - 系统设置、页面配置、标签导入导出、查看器配置等。
- `TrashPage.vue`
  - 回收站主页面；支持滚动浏览与分页浏览两种模式，并会按横竖屏切换等高 / 等宽瀑布流。
