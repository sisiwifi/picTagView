...
- 如果 full-refresh 实际收编了图片，recent 快照会被这批结果覆盖，因此 `/gallery` 的 recent 一级预览和 `/gallery/recent` 二级页都会立即切到这批最新收编结果。
...
- recent 一级预览和 `/gallery/recent` 的直图默认按时间倒序，优先显示最近几张图片。
- 一级预览卡片会在封面资源来自 GIF / 动态 WEBP 时显示 `GIF` / `WEBP` 小标记；详情浮层沿用同一标记。
- 导入状态、进度和结果提示恢复为按需展开的紧凑反馈区，不再在页面空闲时预留固定大块空白。
- 仅在导入或刷新进行中，`GalleryPage.vue` 才会以 2 秒间隔静默轮询 recent overview，避免空闲时持续请求。
...
- 如果已打开详情的图片因为本地筛选失配而不再属于当前网格，详情浮层也不会立即退化成空白状态；它会继续显示这份以 `image_id` 为主锚点的详情快照，直到关闭详情时才统一刷新主网格并重新判断可见性。
...
- 详情浮层里的文件名、主分类、创建时间、Tag 与“↺”同步文件名都先作用在详情快照上，不会立即把当前 browse 数据本地改写到失配状态；用户关闭详情后，`BrowsePage` 才统一重新加载当前契约数据。
- 详情浮层的标签栏除了现有 Tag 编辑入口外，还新增一个“↺”按钮；它会调用 `/api/images/tags/sync-filename`，把当前图片缺失的 `tag.name` 以空格分隔追加到扩展名前。多选时会对每张图片分别计算，不会写成同一个公共文件名。
