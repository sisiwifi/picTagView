...
- 详情浮层打开后会拍一份独立的图片快照，并以 `image_id` 为主锚点维护后续元数据更新；因此某张已选图片即使随后不再命中当前本地筛选，详情仍会继续显示并允许编辑，直到用户关闭详情。
...
- 筛选面板里的标签名称与 description 由共享 Tag lookup 延迟补齐，拉取范围基于完整 `sourceItems` 而不是当前已筛出的 `items`；详情关闭并完成统一 reload 后，新标签的名称与 description 也会跟着补齐。
- 详情浮层里的文件名、主分类、创建时间、Tag 与“↺ 同步文件名”都先只更新详情快照，不会立即本地回写并重判当前网格；真正的 browse 数据会在关闭详情后统一 reload，再由当前筛选决定这些图片是否继续留在网格中。
- 详情浮层的标签栏在原有编辑入口之外，额外提供一个“↺”按钮，调用 `/api/images/tags/sync-filename` 把当前图片缺失的 `tag.name` 以空格分隔追加到扩展名前；多选时逐文件分别处理。
