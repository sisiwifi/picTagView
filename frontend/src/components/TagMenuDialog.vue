      ...
      <div class="tag-menu__body">
        ...
        <div class="tag-menu__block">
          ...
          <div class="tag-menu__entry-actions">
            <button class="tag-menu__btn" type="button" :disabled="busy" @click="$emit('add-new-tag')">添加新标签（预留）</button>
...
export default {
  ...
  methods: {
    chipStyle(tag) {
      const color = normalizeHexColor(tag?.color || '') || '#334155'
      const borderColor = normalizeHexColor(tag?.border_color || '') || color
      const backgroundColor = String(tag?.background_color || '').trim() || hexToRgba(color, 0.4)
      return {
